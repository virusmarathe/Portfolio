...
void drawCircle(float radius)
{
   ...
   for (int i=0; i < 360; i++)
   {
      ...
      glVertex2f(cos(degInRad)*radius,sin(degInRad)*radius-0.5);
   }
 
   glEnd();
}

void showConstraints(struct world *jello)
{
	int i,j,k;
	int N = jello->numConstraintPoints;
	if (viewingMode==0) // render wireframe
    {
		glLineWidth(1);
		glPointSize(5);
		...
		{
	     	glBegin(GL_POINTS); // draw point
			glVertex3f(jello->pc[i].x,jello->pc[i].y,jello->pc[i].z);
			glEnd();
			if (i != N-1)
			{
				glBegin(GL_LINES);
				glVertex3f(jello->pc[i].x,jello->pc[i].y,jello->pc[i].z);
				glVertex3f(jello->pc[i+1].x,jello->pc[i+1].y,jello->pc[i+1].z);
				glEnd();
			}
		}
