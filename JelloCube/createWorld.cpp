...
void writeWorld (char * fileName, struct world * jello)
{
  int i,j,k;
  FILE * file;
  int numConstraintPoints = jello->numConstraintPoints;
...
/*  
  ...
  */
  fprintf(file, "%d\n", jello->numConstraintPoints);
  for (i = 0; i < numConstraintPoints; i++)
  {
	  fprintf(file, "%lf %lf %lf\n", jello->pc[i].x, jello->pc[i].y, jello->pc[i].z);
  }
  for (i = 0; i < numConstraintPoints; i++)
  {
    fprintf(file, "%lf %lf %lf\n", jello->vc[i].x, jello->vc[i].y, jello->vc[i].z);
  }
  fclose(file);
